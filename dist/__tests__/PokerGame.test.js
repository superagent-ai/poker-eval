...
const createMockPlayer = (name, defaultAction) => ({
    name,
    action: async () => defaultAction,
});
const createTestPlayers = (actions) => {
    return actions.map((action, index) => createMockPlayer(`Player ${index + 1}`, action));
};
describe("No Limit Texas Holdem ($5/10)", () => {
    describe("Game Initialization", () => {
        it("should initialize game with correct number of players", () => {
            ...
            const game = new game_1.PokerGame(players);
        ...
        it("should initialize players with correct starting stacks", () => {
            ...
            const game = new game_1.PokerGame(players, { defaultChipSize: 1000 });
    ...
    describe("Game Actions", () => {
        it("should handle basic call actions", async () => {
            ...
            const totalChipsChange = stackDifferences.reduce((sum, diff) => sum + diff, 0);
